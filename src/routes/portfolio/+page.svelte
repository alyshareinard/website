...
<Accordion>
	...
	<div slot="details">
		<h4>
			Created a full inventory and payroll system for a door-to-door sales business that worked with
			their existing sales app. The new app allowed the company to easily record shipments from the
			supplier, products checked out to sales representatives, and items sold to customers. Items
			sold would then be added to the payroll system where base salary, commission and monthly
			bonuses would be calculated. Once approved the system would email the employee a PDF
			summarizing their payroll record.
		</h4>
...
<Accordion>
	...
	<div slot="details">
		<h4>
			...
			up each tracking number daily and updating the SKU.io record manually, so the new system saved
...
<Accordion>
	...
	<div slot="details">
		<h4>
			A client was providing direct marketing incentives to certain subgroups of clients. They used
			Hubspot to track target companies and their associated interests using keywords associated
			with the companies. However, to send out their marketing email they had to find all companies
			associated with a given keyword, and then, by hand go through each of the company employees
			and pull their email addresses. I built a simple app that worked with the Hubspot API to
			complete this process automatically. Using my app the client now just types in a keyword and
			is provided a list of all relevant contacts.
		</h4>
